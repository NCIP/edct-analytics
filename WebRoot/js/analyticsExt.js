...
function checkIfReportTitleExistsCallback(data){
	...
	if ( errorMessageBlk ){
		...
		if ( data ){
			var err = "WARNING: A report template with this title already exists. Saving will override the previously saved report.";
	...
	if ( ! isReportTableReady ) {
		...
		if ( reportQuery ) {
			...
			if ( !isValid ) {
				...
				jAlert("<b><i>Sorry, this query is invalid. Please check your query criteria and try again.</i></b>","Invalid Query");
