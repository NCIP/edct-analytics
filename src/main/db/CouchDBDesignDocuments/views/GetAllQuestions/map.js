function (doc) {
    if (doc.questions) {
        for (var questionId in doc.questions) {
            ...
            if (questionText) emit([questionId, questionSn, questionText, '', ''], null);
        }
    }
	...
	/* SPECIAL FIELDS */
	// Module Id
	if ( doc.moduleId ) emit (['moduleid000000000000000000000000', 'Module', 'The Module Id', '', ''],null);
	
	// Form Id
	if ( doc.formId ) emit (['formid00000000000000000000000000', 'Form', 'The Form Id', '', ''],null);
	
	// Updated Date (timestamp)
	if ( doc.updatedDate ) emit (['updatedDate000000000000000000000', 'UpdatedDate', 'ISO-formatted document update timestamp', '', ''],null);
}